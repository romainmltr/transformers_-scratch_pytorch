...
# Create the LSTM model (remplaçant le RNN)
...
batch_size = 64
learning_rate = 0.001
num_epochs = 5  # Augmenté à 5 pour mieux voir l'évolution
dropout_rate = 0.3  # Ajout d'un dropout pour prévenir le surapprentissage
...
model = LSTM(tokenizer.vocab_size, hidden_size, tokenizer.vocab_size, num_layers, dropout_rate)
...
def train_and_validate(model, train_loader, test_loader, criterion, optimizer, scheduler, epochs):
    model.train()
    ...
    for epoch in range(epochs):
        ...
        # Ajuster le learning rate basé sur la perplexité
        scheduler.step(test_perplexity)
        ...
        if test_perplexity < best_perplexity:
            ...
            torch.save(model.state_dict(), 'shakespeare_lstm_best.pth')
...
torch.save(model.state_dict(), 'shakespeare_lstm_final.pth')
print("Modèle final enregistré sous 'shakespeare_lstm_final.pth'")

# Load the best model
try:
    model.load_state_dict(torch.load('shakespeare_lstm_best.pth'))
...
def test_generation(model, tokenizer, seed_texts, temperatures):
    print("\n=== TESTS DE GÉNÉRATION DE TEXTE (LSTM) ===")
...
test_seeds = [
    ...
    "Friends, Romans, countrymen"  # Ajout d'une nouvelle amorce
]
...
# Comparaison des résultats (si un modèle RNN a été entraîné précédemment)
try:
    # Créer un modèle RNN
    class RNN(nn.Module):
    ...
    # Charger le modèle RNN précédent
    rnn_model = RNN(tokenizer.vocab_size, hidden_size, tokenizer.vocab_size, num_layers)
    try:
        rnn_model.load_state_dict(torch.load('shakespeare_rnn_best1.pth'))
        print("\n=== COMPARAISON RNN vs LSTM ===")
        rnn_loss, rnn_perplexity = calculate_perplexity(rnn_model, test_loader, criterion)
        print(f"Perplexité RNN: {rnn_perplexity:.4f}")
        print(f"Perplexité LSTM: {final_perplexity:.4f}")
        improvement = (rnn_perplexity - final_perplexity) / rnn_perplexity * 100
        print(f"Amélioration LSTM vs RNN: {improvement:.2f}%")
    except:
        print("\nAucun modèle RNN précédent trouvé pour comparaison.")
except:
    print("\nImpossible de créer un modèle RNN pour comparaison.")